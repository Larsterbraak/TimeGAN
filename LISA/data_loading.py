...
def create_dataset(name='EONIA', normalization='outliers',
    ...
    if name == 'EONIA':
        ...
        df.EONIA[1:] = np.diff(df.EONIA) # Take the difference of EONIA
        df = df.iloc[1:, :] # Remove the first entry
    elif name == 'pre-ESTER':
    ...
    if training:
        ...
        X_train = tf.data.Dataset.from_tensor_slices(tf.cast(X_train, tf.float32)).batch(128).prefetch(tf.data.experimental.AUTOTUNE)
        X_test = tf.data.Dataset.from_tensor_slices(tf.cast(X_test, tf.float32)).batch(128).prefetch(tf.data.experimental.AUTOTUNE)
