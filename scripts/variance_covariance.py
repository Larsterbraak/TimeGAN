...
def VaR(r_0, risk_factors, data, time, percentile=0.990, upward=True):
    ...
    if upward:
        return np.ravel(r_0 + time * (intercept + np.matmul(np.transpose(np.ravel(coef)), risk_factors)) + \
               np.sqrt(time) * np.sqrt(np.matmul(np.matmul(np.transpose(np.ravel(coef)),
                                         cov_matrix), np.ravel(coef))) * tdistr.ppf(percentile, 1.9))
    else:
        return np.ravel(r_0 + time * (intercept + np.matmul(np.transpose(np.ravel(coef)), risk_factors)) - \
               np.sqrt(time) * np.sqrt(np.matmul(np.matmul(np.transpose(np.ravel(coef)),
                                   cov_matrix), np.ravel(coef))) * tdistr.ppf(percentile, 1.9))
...
VaR(eonia[-1], current_situation, df, 20, 0.990, False)
...
for i in range(250):
    upper += eonia[3546+i] > VaR(eonia[3545+i], np.array(df.drop('EONIA', axis=1).iloc[3545+i]), df.iloc[3545-250+i:3545+i,:], 1, 0.990, True)
    lower += eonia[3546+i] < VaR(eonia[3545+i], np.array(df.drop('EONIA', axis=1).iloc[3545+i]), df.iloc[3545-250+i:3545+i,:], 1, 0.990, False)
